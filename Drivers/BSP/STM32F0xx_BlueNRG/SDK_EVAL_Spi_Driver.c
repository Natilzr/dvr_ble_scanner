...
#include "gp_timer.h"
//#include "stm32F0xx_hal_bluenrg_spi.h"
...
for(volatile int i = 0; i < 340; i++)__NOP();
#else
...
{
  GPIO_InitTypeDef  GPIO_InitStruct;
  uint8_t param_write[14]={0x01, 0x27, 0xFC, 0x0A, 0x98, 0x03, 0x01, 0x10, 0x04, 0x00, 0x00, 0x34, 0x5B, 0x00};
  
  BlueNRG_HW_Bootloader();
  if (isHWBootloader_Patched())
    return;


  /* Critical section executed with SPI interrupts disabled */
  Disable_SPI_IRQ();
  ...
  HAL_GPIO_WritePin(BNRG_SPI_RESET_PORT, BNRG_SPI_RESET_PIN, GPIO_PIN_RESET);
  /* TBD: it should be better to replace this Clock_Wait with an implementation that does not requires
     interrupts */
  /* 5 ms delay */
  HAL_Delay(5);
  HAL_GPIO_WritePin(BNRG_SPI_RESET_PORT, BNRG_SPI_RESET_PIN, GPIO_PIN_SET);
 
  while (BlueNRG_SPI_Write_Raw(param_write, sizeof(param_write)) != 0); 
...
uint8_t BlueNRG_SPI_Read_All(uint8_t *buffer, uint8_t buff_size)
{
  ...
  HAL_SPI_TransmitReceive(&SpiHandle, (uint8_t*)header_master, header_slave, HEADER_SIZE,10);
  if (header_slave[0] == 0x02) {
    // device is ready
#if DEBUG_P    
   printf("read %d %d %d %d %d\n\r",header_slave[0],header_slave[1],header_slave[2],header_slave[3],header_slave[4]);
#endif
    ...
    if (byte_count > 0) {
      ...
      HAL_SPI_TransmitReceive(&SpiHandle, &char_ff, buffer, byte_count, 10);
#if DEBUG_P
      printf("count  %d \n\r",byte_count);
#endif
      len = byte_count;
...
{  
  ...
  //HAL_SPI_TransmitReceive_Opt(&SpiHandle, header_master, header_slave, HEADER_SIZE);
  HAL_SPI_TransmitReceive(&SpiHandle, (uint8_t*)header_master, header_slave, HEADER_SIZE,100);
 ...
 printf("send1 %d \n\r",Nb_bytes1);
  ...
  HAL_SPI_TransmitReceive(&SpiHandle, data1, read_char_buf,Nb_bytes1,10);
  rx_bytes -= Nb_bytes1;
  ...
  HAL_SPI_TransmitReceive(&SpiHandle, data2, read_char_buf,tx_bytes,10);
#if DEBUG_P
   printf("send2 %d \n\r",tx_bytes);
#endif   
  result = tx_bytes;
